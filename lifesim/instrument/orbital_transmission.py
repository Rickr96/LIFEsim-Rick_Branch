...
class OrbitalTransmissionMap(TransmissionMap):
    ...
    def projected_vector(self, theta, inc, rad):
        """
        Calculates the 3d rotated star to planet vector from inclination and true anomaly.
    ...
    def get_transmission_curve(self, index, time_dependent=True, phi_n=360):
        ...
        time_dependent: bool
            True --> Instrument has a rotating array, meaning that projected_vector must be translated
            to the time dependent rotation of the array.
            False --> Non-rotating Array, the projected_vector can be used as is (still time dep.)
        phi_n: int
